...
func _ready() -> void:
	...
	data["import"] = "haystack.components.converters.txt.TextFileToDocument"
	data["component_name"] = "txt"
	data["receivers"] = ["txt.sources"]
	data["senders"] = ["txt.documents"]
...
func conn_request(from:StringName,from_port:int,to:StringName,to_port:int)->void:
	...
	if origin.data.has("output_types"):
		...
		if output_types.find("text/plain") != from_port:
