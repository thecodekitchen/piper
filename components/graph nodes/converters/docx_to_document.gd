...
func _ready() -> void:
	...
	data["import"] = "haystack.components.converters.docx.DOCXToDocument"
	data["deps"] = ["python-docx"]
	data["component_name"] = "docx"
	data["receivers"] = ["docx.sources"]
	data["senders"] = ["docx.documents"]
...
func conn_request(from:StringName,from_port:int,to:StringName,to_port:int)->void:
	...
	if origin.data.has("output_types"):
		...
		if output_types.find("application/vnd.openxmlformats-officedocument.wordprocessingml.document") != from_port:
