...
func _ready() -> void:
	...
	data["import"] = "haystack.components.converters.pypdf.PyPDFToDocument"
	data["deps"] = ["pypdf"]
	data["component_name"] = "pdf"
	data["receivers"] = ["pdf.sources"]
	data["senders"] = ["pdf.documents"]
func conn_request(from:StringName,from_port:int,to:StringName,to_port:int)->void:
	...
	if origin.data.has("output_types"):
		...
		if output_types.find("application/pdf") != from_port:
